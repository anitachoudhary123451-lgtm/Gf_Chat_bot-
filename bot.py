...
# ============================================================
# DATABASE CORE (INDENTATION STRICTLY FIXED)
# ============================================================
...
def user_profile(message):
    ...
    if len(parts) < 2:[span_198](start_span)[span_198](end_span)
        bot.send_message(ADMIN_ID, "⚠️ <b>Format:</b> <code>/userprofile &lt;user_id&gt;</code>")[span_199](start_span)[span_199](end_span)
        return
    ...
    if not user_id.lstrip("-").isdigit():[span_201](start_span)[span_201](end_span)
        bot.send_message(ADMIN_ID, "⚠️ Invalid Chat ID.")[span_202](start_span)[span_202](end_span)
        return
    ...
    if user_id not in data.get("users", {}):[span_204](start_span)[span_204](end_span)
        if user_id == PROTECTED_USER_ID:[span_205](start_span)[span_205](end_span)
            ...
            bot.send_message(ADMIN_ID, f"⚠️ User <code>{user_id}</code> is not registered.")[span_208](start_span)[span_208](end_span)
            return
    u = data["users"].get(user_id, {})[span_209](start_span)[span_209](end_span)
    if user_id in data["blocked"]:[span_210](start_span)[span_210](end_span)
        status = "⛔ Banned[span_211](start_span)"[span_211](end_span)
    elif user_id in data["alerts"]:[span_212](start_span)[span_212](end_span)
        status = "🚨 Alert[span_213](start_span)"[span_213](end_span)
    elif user_id == PROTECTED_USER_ID:
        status = "🛡️ Protected"
    else:
        status = "🟢 Active[span_214](start_span)"[span_214](end_span)
...
def start_services():
    ...
    try:
        ...
        logging.info("Webhook cleared, waiting 5 sec to resolve instance conflicts...")[span_337](start_span)[span_337](end_span)
    except Exception as e:
        logging.warning(f"Webhook clear warning: {e}")[span_338](start_span)[span_338](end_span)
